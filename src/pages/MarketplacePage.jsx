    ...
    try {
      ...
      if (ncrTradeModal.mode === 'buy') {
        response = await marketplaceService.buyFromNcr({
          ...
          quantity: ncrTradeModal.quantity
        }, token);
      } else {
            ...
            <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50 animate-fade-in">
              <div className="glass-card max-w-sm w-full p-6 border border-cyanGlow/20 bg-gradient-to-b from-glassBg to-black/95 relative overflow-hidden font-mono text-xs text-white">
                ...
                <form onSubmit={handleNcrTrade} className="space-y-4">
                  ...
                  <div>
                    <label htmlFor="ncr-qty" className="block text-[10px] text-text-secondary uppercase mb-1 font-sans">Quantity</label>
                    <input
                      id="ncr-qty"
                      type="number"
                      min="1"
                      max={mode === 'sell' ? availableStock : undefined}
                      value={ncrTradeModal.quantity}
                      onChange={(e) => setNcrTradeModal(prev => ({ ...prev, quantity: Math.max(1, parseInt(e.target.value) || 1) }))}
                      className="w-full bg-black/60 border border-white/10 rounded px-3 py-2 text-white focus:outline-none focus:border-cyanGlow/50"
                      required
                    />
                  </div>
